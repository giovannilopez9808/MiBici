...
parameters = {"path data": "../Output/",
              "file data": "Hourly_var_time_travel.csv",
              "path graphics": "../Graphics/",
              "file graphics": "monthly_hourly_var_time_travel.png"}
...
data_mean = np.sqrt(np.array(data_mean))
...
for ytick in yticks:
    plt.plot([xticks[0], 16.5],
             ...
             alpha=0.5,
             lw=1.5)
cbar = plt.colorbar(ticks=np.linspace(0, 4700, 11))
cbar.set_label("Desviación estandar del tiempo por viaje (min)",
               rotation=-90,
               labelpad=15)
plt.tight_layout()
